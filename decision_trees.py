...
X_train, X_test, y_train, y_test = train_test_split(load_breast_cancer().data, load_breast_cancer().target, stratify=load_breast_cancer().target, random_state=42)
cancer_tree = DecisionTreeClassifier(random_state=0, min_samples_leaf=15).fit(X_train, y_train)
print('Accuracy of Decision tree is {:.2f}'.format(100*cancer_tree.score(X_test, y_test)))
print('Accuracy of Decision tree (training) is {:.2f}'.format(100*cancer_tree.score(X_train, y_train)))

#after adding max depth (4), accuracy of predictions increases 93.71% to 95.10%
#if using min_leaf_samples (15), accuracy of predictions increases to 95.80%
...
#displays an easy-to-understand visualization of the decision tree
#print("Feature Importance:\n{}".format(cancer_tree.feature_importances_))
#print('Feature vs Importance')

#for feature, importance in zip(load_breast_cancer().feature_names, cancer_tree.feature_importances_):
 #   print('{} - {:.2f}%'.format(feature, 100*importance))
